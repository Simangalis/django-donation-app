        ...
        try:
            ...
            if user:
                ...
                if donor_user:
                    ...
                    messages.warning(request,'Invalid Door User')
            else:
                messages.warning(request,'Invalid username and password')
        except:
            messages.warning(request,'Login Failed')
...
def donationnotrec_admin(request):
    if not request.user.is_authenticated:
        return redirect('/login-admin')
    donation = Donation.objects.filter(status='Volunteer Not Received')
...
def index_donor(request):
    ...
    rejectedcount = Donation.objects.filter(donor=donor,status="rejected").count()
...
def index_volunteer(request):
    ...
    totalNotRecDelivered = Donation.objects.filter(volunteer=volunteer,status="Donation NotReceived").count()
